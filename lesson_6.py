...
class Warrior(Heroes):
    ...
    def apply_super_power(self, boss, heroes):
        ...
        print(f"Удар увеличился в {critical}")
...
def print_statistic(boss, heroes):
    print(f"_________________ROUND {round_counter}________________")
    print(boss)
    for hero in heroes:
        print(hero)
...
def start_game():
    boss = Boss("Syimyk", 500, 20)
    warrior = Warrior("Abu", 100, 20)
    medic = Medic("Rayona", 65, 0)
